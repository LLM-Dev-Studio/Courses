...
export function QuizQuestionCard({
  ...
  return (
    <div className="rounded-xl border border-[var(--sand-300)] bg-[var(--sand-50)] p-4">
      <p className="mb-2 text-sm font-semibold text-[var(--green-900)]">
        Question {questionNumber} of {totalQuestions}
      </p>
      <p className="mb-3 text-sm text-[var(--green-800)]">{question.prompt}</p>
            ...
            <button
              ...
              className={`flex w-full items-start justify-between gap-2 rounded-lg border px-3 py-2 text-left text-sm transition ${
                showCorrectHighlight
                  ? "border-[var(--green-700)] bg-[#eaf4ed] text-[var(--green-900)]"
                  : showWrongSelection
                    ? "border-[#b83e3e] bg-[#fdecec] text-[#7a1f1f]"
                    : isSelected
                      ? "border-[var(--green-700)] bg-white text-[var(--green-900)]"
                      : "border-[var(--sand-300)] bg-white text-[var(--green-800)] hover:border-[var(--sand-400)]"
              }`}
            >
      ...
      {feedbackMessage ? (
        <div className="mt-4">
          <p className="text-sm text-[var(--green-800)]">{feedbackMessage}</p>
        </div>
      ) : null}
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <button
          ...
          className="rounded-lg border border-[var(--sand-400)] px-3 py-1.5 text-sm font-semibold text-[var(--green-800)] disabled:cursor-not-allowed disabled:opacity-40"
        >
          Previous Question
        </button>

        <div className="flex items-center gap-2">
          <p className="text-xs font-semibold text-[var(--green-700)]">{attemptDisplay}</p>
          <button
            type="button"
            onClick={onProceed}
